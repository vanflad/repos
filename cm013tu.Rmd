...
ggplot(gapminder, aes(x=lifeExp))+
  geom_histogram(binwidth = 5, aes(fill=continent))+
  facet_wrap(~continent)+
  scale_fill_manual(values=brewer.pal(n=5, "Dark2")) #my answer

ggplot(gapminder, aes(x=lifeExp))+
  facet_wrap(~continent)+
  geom_histogram(y=..density.., aes(fill=continent))+
  scale_fill_brewer(palette = "Dark2")
#vincenzo's answer. error object "..density.." not found...
```
...
```{r}
color_layer <- scale_color_manual("", labels=c("Other Countries", "Rwanda"), values=c("black", "red"))
#included "" because didn't want legend to have a title
ggplot(gapminder, aes(year, lifeExp))+
  geom_line(aes(group=country, color=country=="Rwanda"), alpha=0.2)+
  color_layer+
  scale_alpha_discrete(range=c(0.2, 1), guide=FALSE)
#answer, didn't have time to answer myself, did it as a demo.
#couldn't get the last line of code to work tho... wtf.
```
